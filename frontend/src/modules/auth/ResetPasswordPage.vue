...
<template>
  <div class="flex min-h-screen items-center justify-center px-4 py-10">
    <div class="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-300/20 dark:border-slate-800 dark:bg-slate-900">
      <h1 class="text-2xl font-semibold text-slate-950 dark:text-slate-100">Choose a new password</h1>

      <InlineAlert v-if="errorMessage" title="Reset failed" :description="errorMessage" tone="danger" class="mt-5" />
      <InlineAlert v-if="successMessage" title="Password updated" :description="successMessage" tone="success" class="mt-5" />
      ...
      <form class="mt-6 space-y-4" @submit.prevent="submit">
        ...
        <input v-model="token" required placeholder="Reset token" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
        <input v-model="password" required type="password" autocomplete="new-password" placeholder="New password" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
        <input v-model="passwordConfirmation" required type="password" autocomplete="new-password" placeholder="Confirm new password" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
