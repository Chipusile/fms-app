...
<template>
  <div class="space-y-6">
    <PageHeader eyebrow="Partners" :title="isEditMode ? 'Edit service provider' : 'Create service provider'" description="Centralise external garages, insurers, and support vendors for later operational workflows.">
      <template #actions>
        <RouterLink :to="{ name: 'service-providers' }" class="inline-flex items-center rounded-2xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
          Cancel
        </RouterLink>
      </template>
    </PageHeader>
      ...
      <SectionCard title="Provider details" description="Identity and contact details for external partners.">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <label class="space-y-2 text-sm text-slate-700">
            ...
            <select v-model="form.provider_type" class="w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500" :disabled="loading">
          ...
          <label class="space-y-2 text-sm text-slate-700">
            ...
            <input v-model="form.contact_person" class="w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500" :disabled="loading">
      ...
      <SectionCard title="Commercial details" description="Administrative metadata for compliance and procurement teams.">
        <div class="space-y-4 text-sm text-slate-700">
          <label class="space-y-2">
            ...
            <input v-model="form.tax_number" class="w-full rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-blue-500" :disabled="loading">
          ...
          <div class="flex flex-col gap-3 sm:flex-row">
            <RouterLink :to="{ name: 'service-providers' }" class="inline-flex w-full items-center justify-center rounded-2xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 sm:w-auto">
              Cancel
            </RouterLink>
